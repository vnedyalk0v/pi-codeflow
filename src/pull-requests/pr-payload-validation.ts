...
function getPrPayloadSchemaPath(): string {
  return fileURLToPath(new URL('../../schemas/pr-payload.schema.json', import.meta.url));
}

function mapValidationErrors(errors: ErrorObject[]): CodeflowPrValidationIssue[] {
  return errors.map((error) => {
    const path = getErrorPath(error);
    const allowedValues = getAllowedValues(error);
    const mappedError: CodeflowPrValidationIssue = {
      path,
